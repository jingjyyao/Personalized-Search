"""
数据集文件记录：(1)数据集所在路径：/home/songwei_ge/personalize/ProcessedData（93）
(2)用户的查询日志：ValidQLSample_HF目录下，每个文件代表一个用户，每条记录包括(keywords, queryid, sessionid, date, url, SAT, urlrank0/1)，用\t分隔
(3)QueryVector_xxx：对应模型生成的query向量表示，共有1624505个query
(4)DocVectot_xxx：对应模型生成的doc向量表示，共有681512篇文档表示

准备训练数据：以一个session为一个episode
# reward不应该基于原来的rank计算，而是应该基于实际交互中计算得到的rank列表来进行计算
# 在用户画像的部分考虑采用文档对的形式，同时合并了positive和negative的信息
"""
...
"""
    word2vec: Deepwalk, GoogleNews, Wiki, QueryLog
"""

class Dataset:
    VALID_WORD2VEC = {'Deepwalk', 'GoogleNews', 'Wiki', 'QueryLog'}
    FEATURE_SIZE = {'Deepwalk':300, 'GoogleNews':300, 'Wiki':1000, 'QueryLog':300}
    def __init__(self, data_path='/home/songwei_ge/personalize/ProcessedData/',
                querylogfile='ValidQLSample_HF', word2vec='GoogleNews', limitation=10000000,
                max_query_num=300, min_session=4, max_doc_num=10, max_pair_num=20, other_feature_size=110,
                vector_path='/home/songwei_ge/personalize/ProcessedData/',
                batch_size=200):
        """
        初始化数据集中需要用到的参数
        """
        ...
        self.in_path = '/home/jing_yao/learning/personalization/NewData/SampleUsers'
        # self.in_path = '/home/yujia_zhou/refind/data/ValidQLSample_HF_new'
        self.filenames = sorted(os.listdir(self.in_path))
        self.query2vec_path = os.path.join(vector_path, '_'.join(['QueryVector', word2vec]))
        self.doc2vec_path = os.path.join(vector_path, '_'.join(['DocVector', word2vec]))
        self.feature_size = Dataset.FEATURE_SIZE[word2vec]
    ...
    def init_dict(self):
        """
        创建query和url到id的映射，字典中的元素分别为"query:id"或"url:id"
        """
        self.query2id = dict()
        self.url2id = dict()
        with open(os.path.join(self.data_path, 'query2id.txt')) as fr:
            for line in fr:
                idx, query = line.strip().split('\t')
                self.query2id[query.strip().lower()] = idx
        with open(os.path.join(self.data_path, 'url2id.txt')) as fr:
            for line in fr:
                idx, url = line.strip().split('\t')
                self.url2id[url.strip().lower()] = idx
        print('Successfully created the dictionary of query2id and url2id.')


    def transform_q(self, sentence):
        """
        将每个query转化为id，再返回对应的queryvector
        """
        idx = self.query2id[sentence.strip().lower()]
        return pickle.load(open(os.path.join(self.query2vec_path, idx+'.pkl'), 'rb'))


    def transform_u(self, sentence):
        """
        将每个url转化为id，再返回对应的docvector         
        """
        idx = self.url2id[sentence.strip().lower()]
        return pickle.load(open(os.path.join(self.doc2vec_path, idx+'.pkl'), 'rb'))


    def divide_dataset(self, filename):
        """
        统计每个用户的queryLog中query和session的数目，并判断这个用户的数据是否有效，舍弃数据少于一定程度的用户
        """
        self.session_sum = 0
        ...
        with open(os.path.join(self.in_path, filename)) as fhand:
            for line in fhand:
                try:
                    ...
                    line = line.strip()
                _, _, sess_time, _, query_id, query, query_time, _, urlrank, url, _, _, click=line.split('\t')
                if float(click)>=30:
                    sat='1'
                else:
                    sat='0'
                date = sess_time.split(' ')[0]
                queryid = query_id + '_' + query + '_' + query_time
                sessionid=sess_time
                ...
                if current_date < self.cutoff_date:
                    ...
                    if int(sat) == 1:
                        query_sum += 1
            #print('session_sum:',self.session_sum)
            if self.session_sum < self.min_session:
                return False
            if self.session_sum < 4:
                self.session_test = 1
                self.session_valid = 1
                self.session_train = self.session_sum - 2
            else:
                self.session_test = int(self.session_sum/5)
                self.session_valid = int((self.session_sum-self.session_test)/5)
                self.session_train = self.session_sum - self.session_test - self.session_valid
        return True
    ...
    def prepare_dataset(self):
        ...
        for filename in self.filenames:
            ...
            for line in fr:
                ...
                _, _, sess_time, _, queryid, query, querytime, _, urlrank, url, _, _, click=line.split('\t')
                if float(click)>=30:
                    sat='1'
                else:
                    sat='0'
                date = sess_time.split(' ')[0]
                queryid = queryid + '_' + query + '_' + querytime
                sessionid=sess_time
                ...
                if current_date < self.cutoff_date: # 之前的查询作为用户的历史查询
                    if queryid != last_queryid:
                        query_count += 1
                        if len(last_query['urls']) > 0:  # 上一个查询的全部记录
                            ...
                            for index, (label, pair, _) in enumerate(pairs): # 用label标记这个pair是+还是-，还是(+,-)
                                ...
                                if label == '+': # pair中只有正例
                                    history_pairs[index][self.feature_size: 2*self.feature_size] += self.transform_u(pair[0].lower())
                                    history_pairs[index][-1] = 0.1
                                elif label == '-': # pair中只有负例
                                    history_pairs[index][2*self.feature_size: 3*self.feature_size] += self.transform_u(pair[0].lower())
                                    history_pairs[index][-1] = 0.1
                                else:
                                    history_pairs[index][self.feature_size: 2*self.feature_size] += self.transform_u(pair[0].lower())
                                    history_pairs[index][2*self.feature_size: 3*self.feature_size] += self.transform_u(pair[1].lower())
                    ...
                    if queryid != last_queryid: # 开始一个新的query
                        ...
                        last_queryid = queryid
                        last_querytime = querytime
                    one_query['doc_feature'].append(self.transform_u(url.lower()))
        ...
        for filename in self.filenames:
            ...
            for line in fr:
                ...
                if current_date < self.cutoff_date: # 之前的查询作为用户的历史查询
                    ...
                    one_query['position'].append(int(urlrank))
                    one_query['other_feature'].append(features)
                    one_query['lines_test'].append(line+'\t'+str(session_count_all))
                    ...
                    #if int(sat) == 1:
                    #    history[-1][self.feature_size: 2*self.feature_size] += self.transform_u(url.lower())
            if len(one_query['label'])!=0 and session_count >= self.session_train + self.session_valid and len(one_query['history']) > 0:  # 注意最后一个查询、最后一个session的处理
                self.testset.append(one_query)
            fr.close()
