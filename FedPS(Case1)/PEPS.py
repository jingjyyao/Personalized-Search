# 实现PEPS模型，包括global word embedding + personal word embedding + ranking model
import time
import pickle
...
class PEPS:
    def __init__(self, args):
        ...
        self.person_vocab_size = args.person_vocab_size
        ...
        self.start_token = self.person_vocab_size
        self.end_token = self.person_vocab_size + 1
        self.person_vocab_size += 2
    ...
    def load_embedding(self, embedding_path, vocab_path, vocabulary_size): # load the pretrained embedding
        ...
        embed = np.random.uniform(low=-1, high=1, size=(vocabulary_size, self.embedding_size))
        embed[0] = np.zeros((1, self.embedding_size))  # pad的embedding为0
        with open(embedding_path, 'r') as fr:
            for line in fr:
                line = line.strip().split()
                if line[0] not in vocabulary:
                    continue
    ...
    def build_model(self):  # 搭建整个模型
        ...
        # initialize the personal embedding for the user，后面会为对应用户做个人初始化
        if self.embedding_path:
            embed = self.load_embedding(self.embedding_path, self.vocab_path, self.person_vocab_size) # 在第一种情景下，我们在客户端不修改global word embedding
            self.person_embeddings = tf.Variable(tf.constant(embed, dtype='float32', shape=[self.person_vocab_size, self.embedding_size]), name='person_embed', trainable=False)            
        else:
            self.person_embeddings = tf.Variable(tf.random_uniform([self.person_vocab_size, self.embedding_size]), name='person_embed', trainable=False)

        # initialize the embedding
        if self.embedding_path:
            embed = self.load_embedding(self.embedding_path, self.vocab_path, self.vocabulary_size) # 在第一种情景下，我们在客户端不修改global word embedding
            self.embeddings = tf.Variable(tf.constant(embed, dtype='float32', shape=[self.vocabulary_size, self.embedding_size]), name='global_embed', trainable=False)
            print("Initialized embeddings with pretrained in {0}".format(self.embedding_path))
        else:
            self.embeddings = tf.Variable(tf.random_uniform([self.vocabulary_size, self.embedding_size]), name='global_embed', trainable=False)
        ...
        self.q_embed_personal = tf.nn.embedding_lookup(self.person_embeddings, self.input_q_personal, name='q_emb_personal')
        ...
        # compute the contextual representation
        ...
        with tf.variable_scope("query_attention", reuse=True):
        ...
        with tf.variable_scope("attend_personal_personal"):
            ...
            print("shape of attend similarity matrix: ", self.pos_similarity_attend_personal.shape)  #[batchsize, qlen, dlen]
        ...
        with tf.variable_scope("query_suggestion"):
            ...
            self.decoder_inputs_embedded = tf.nn.embedding_lookup(self.person_embeddings, self.decoder_inputs, name='decoder_inputs_embedded')
        ...
        with tf.variable_scope("Final_Score"):
            # final_pos_score = tf.concat([self.pos_kernel_score_1, self.pos_kernel_score_2, self.pos_feature_score], -1)
        ...
        with tf.variable_scope("Final_Score", reuse=True):
            # final_neg_score = tf.concat([self.neg_kernel_score_1, self.neg_kernel_score_2, self.neg_feature_score], -1)
    ...
    def train(self, dataset):  # 训练模型
        ...
        for idx, epoch_data in enumerate(dataset.gen_epochs()):
            ...
            for batch_data in epoch_data:
                ...
                #exit()
                #print("nan in gradient: ", np.isnan(gradient).any())
                #print("aggregate positive kernel: ", pos_aggre[0])
                #print("aggregate negative kernel: ", neg_aggre[0])
                #print("training scores: ", scores[0])
                ...
                valid_loss_, valid_acc_ = self.sess.run([self.loss, self.accuracy], feed_dict_valid)
